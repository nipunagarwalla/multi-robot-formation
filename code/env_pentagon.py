...
# -*- coding: utf-8 -*-
"""
Created on Sat May 25 00:12:09 2024
...
class PassageEnv(gym.Env):
    def __init__(self, config):
        ...
        self.obstacles = [
            ...
            {"min": [-1.5,-1],"max": [-1.1,-0.5],},
            {"min": [-1.4,-0.5],"max": [-1.0,0],},
            {"min": [-1.3,0],"max": [-0.9,0.5],},
            {"min": [-1.2,0.5],"max": [-0.8,1],},

            # up wall
            {"min": [1.2,-1.5],"max": [1.6,-1],},
            {"min": [1.1,-1],"max": [1.5,-0.5],},
            {"min": [1.0,-0.5],"max": [1.4,0],},
            {"min": [0.9,0],"max": [1.3,0.5],},
            {"min": [0.8,0.5],"max": [1.2,1],},
        ]
    ...
    def get_starts_and_goals(self, n):
        ...
        starts[:, :, Y] += rand_n_agents(-1.8, -1.7)

        goals = formation.repeat(n, 1, 1)
        goals[:, :, X] += rand_n_agents(0, 0.1)
        goals[:, :, Y] += rand_n_agents(1.7, 1.8)
    ...
    def vector_step(
        ...
        # Make sure the alignment of each pair of robots
        rewards[:, 4] -= torch.abs(robot_1_position_y - robot_4_position_y)
        rewards[:, 3] -= torch.abs(robot_2_position_y - robot_3_position_y)
        rewards[:, 2] -= torch.abs(robot_2_position_y - robot_3_position_y)
        rewards[:, 1] -= torch.abs(robot_1_position_y - robot_4_position_y)
        #########################################################################
        # Calculate the center distance(radius)
        expected_radius_length = 0.6
        ...
        self.states[(self.states == STATE_INITIAL) & (y_difference_ini < 0.6)] += 1
        self.states[(self.states == STATE_PASSAGE) & (y_difference_pas < 0.2)] += 1
        self.states[(self.states == STATE_AFTER) & (rew_vecs_norm < 0.1)] += 1
        #######################################################################
        ...
        rewards[self.states == STATE_REACHED_GOAL] += 15.0
