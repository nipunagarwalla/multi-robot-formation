...
- **`hallway_node`** — runs the **`policy_v2`**
  checkpoint (`MAX_AGENTS = 4`, square / triangle / line / solo
  formation).
- **`circle_node`** — runs the **`circle_policy_v1`**
  checkpoint (`MAX_AGENTS = 10`, n-gon "circle" formation).
...
≤ 4 robots, square / triangle / line / solo formations.
...
ros2 run limo_circle_sim hallway_node --ros-args \
    -p weights:=/abs/path/to/policy.pt \
    -p num_agents:=4 \
    -p total_robots:=4
```
...
≤ 10 robots, n-gon "circle" formation.
